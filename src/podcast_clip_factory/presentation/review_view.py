...
class ReviewView(ft.Column):
    def __init__(self) -> None:
        ...
        self.y_value_text = ft.Text("96", size=12, color=ft.Colors.BLUE_GREY_600)
        ...
        self.impact_y_value_text = ft.Text("1520", size=12, color=ft.Colors.BLUE_GREY_600)
                    ...
                    controls=[
                        ...
                        ft.Row(
                            ...
                            controls=[
                                ...
                                ft.Container(expand=True, content=self.y_slider),
                                ft.IconButton(
                                    icon=ft.Icons.ADD,
                                    icon_size=16,
                                    on_click=lambda _e: self._nudge_y(2),
                        ...
                        ft.Row(
                            ...
                            controls=[
                                ...
                                ft.Container(expand=True, content=self.impact_y_slider),
                                ft.IconButton(
                                    icon=ft.Icons.ADD,
                                    icon_size=16,
                                    on_click=lambda _e: self._nudge_impact_y(2),
    ...
    def _on_title_change(self, e: ft.ControlEvent) -> None:
        self.preview_title_text.value = (e.control.value or "").strip()[:28] or "タイトルプレビュー"
        self._sync_preview()

    def _on_impact_change(self, e: ft.ControlEvent) -> None:
        self.preview_impact_text.value = (e.control.value or "").strip()[:40] or "インパクト一言プレビュー"
        self._sync_preview()
    ...
    def _nudge_y(self, delta: int) -> None:
        cur = int(self.y_slider.value or 96)
        self.y_slider.value = max(int(self.y_slider.min), min(int(self.y_slider.max), cur + delta))
        self._sync_preview()

    def _nudge_impact_y(self, delta: int) -> None:
        cur = int(self.impact_y_slider.value or 1520)
        self.impact_y_slider.value = max(
            int(self.impact_y_slider.min),
            min(int(self.impact_y_slider.max), cur + delta),
        )
        self._sync_preview()
    ...
    def _sync_preview(self) -> None:
        ...
        self.preview_title_box.bgcolor = (
            ft.Colors.with_opacity(0.55, ft.Colors.BLACK) if bool(self.bg_checkbox.value) else None
        )
        self.preview_title_box.padding = 8 if bool(self.bg_checkbox.value) else 0
        ...
        self.preview_impact_box.bgcolor = (
            ft.Colors.with_opacity(0.55, ft.Colors.BLACK) if bool(self.impact_bg_checkbox.value) else None
        )
        self.preview_impact_box.padding = 8 if bool(self.impact_bg_checkbox.value) else 0
