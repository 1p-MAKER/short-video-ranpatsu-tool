...
def test_ffmpeg_command_contains_letterbox_filter_and_codec():
    ...
    assert any("ass='sub.ass'" in token for token in cmd)


def test_ffmpeg_command_can_skip_subtitles():
    cfg = RenderConfig(
        video_width=1080,
        video_height=1920,
        center_width=1080,
    ...
    cmd = builder.build(Path("in.mp4"), Path("out.mp4"), None, candidate)

    assert "-filter_complex" in cmd
    assert any("drawtext=" in token for token in cmd)
    assert not any("ass='" in token for token in cmd)
